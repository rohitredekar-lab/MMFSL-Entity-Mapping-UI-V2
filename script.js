...
function initCreateEntityForm() {
    ...
    let currentLevels = ['Region', 'State', 'Circle', 'Branch']; // Default initial state
    ...
    const renderLevels = () => {
        ...
        currentLevels.forEach((val, i) => {
            const isLast = (i === currentLevels.length - 1);
            const row = document.createElement('div');
            ...
            row.innerHTML = `
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
                    <label style="font-size:12px; font-weight:700; color:#4b5563;">Level ${i + 1} Name</label>
                    <div style="display:flex; align-items:center; gap:8px;">
                        ${isLast ? '<span style="font-size:10px; color:#e41837; font-weight:700; text-transform:uppercase; letter-spacing:0.5px; margin-right:8px;">Mandatory Base</span>' : `
                            <div class="hierarchy-row-btns">
                                <button class="hierarchy-row-btn remove" data-index="${i}" title="Remove Level">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><line x1="5" y1="12" x2="19" y2="12"/></svg>
                                </button>
                                <button class="hierarchy-row-btn add" data-index="${i}" title="Add Level Below">
                ...
                <select class="cet-input cet-select geo-level-select" data-level="${i + 1}" 
                    ...
                    ${GEOGRAPHY_OPTIONS.map(o => `<option value="${o}" ${val === o ? 'selected' : ''}>${o}</option>`).join('')}
                </select>
            `;
        ...
        // Update global count
        if (countInput) countInput.value = currentLevels.length;
        if (displayEl) displayEl.textContent = currentLevels.length;

        setupLevelListeners();
        updateLivePreview();
    };

    const setupLevelListeners = () => {
        const selects = geoContainer.querySelectorAll('.geo-level-select');
        selects.forEach((sel, idx) => {
            sel.onchange = () => {
                currentLevels[idx] = sel.value;
                // Unique check
                const chosen = currentLevels.filter(Boolean);
                selects.forEach((s, sIdx) => {
        ...
        geoContainer.querySelectorAll('.hierarchy-row-btn.add').forEach(btn => {
            btn.onclick = () => {
                const idx = parseInt(btn.dataset.index);
                if (currentLevels.length < 6) {
                    currentLevels.splice(idx + 1, 0, "");
                    renderLevels();
                } else {
                    showCustomModal({ title: 'Limit Reached', message: 'Maximum 6 hierarchy levels allowed.', type: 'info' });
                }
            };
        });

        geoContainer.querySelectorAll('.hierarchy-row-btn.remove').forEach(btn => {
            btn.onclick = () => {
                const idx = parseInt(btn.dataset.index);
                if (currentLevels.length > 2) {
                    ...
                    showCustomModal({ title: 'Minimum Reached', message: 'At least 2 hierarchy levels are required.', type: 'info' });
                }
            };
        });
    };
    ...
    function updateLivePreview() {
        ...
        const values = currentLevels.map((lvl, i) => lvl || `Level ${i + 1}`);
    ...
    if (minusBtn) minusBtn.onclick = () => {
        if (currentLevels.length > 2) {
            currentLevels.splice(currentLevels.length - 2, 1); // Remove level above Branch
            renderLevels();
        }
    };
    if (plusBtn) plusBtn.onclick = () => {
        if (currentLevels.length < 6) {
            currentLevels.splice(currentLevels.length - 1, 0, ""); // Add level above Branch
            renderLevels();
        }
    };
...
window.showUpdateEntity = function (name, idx) {
    ...
    // --- Per-row +/- controls for Update Hierarchy ---
    ...
    // Mutable levels array, initialised from entity data
    let updCurrentLevels = [...ent.geoLevels];

    const syncUpdCount = () => {
        if (countInput) countInput.value = updCurrentLevels.length;
        if (displayEl) displayEl.textContent = updCurrentLevels.length;
    };
    ...
    const renderUpdateUI = (levels) => {
        ...
        if (container) {
            ...
            levels.forEach((lvl, i) => {
                ...
                row.style.cssText = 'background:white; border:1px solid #eee; border-radius:12px; padding:16px; margin-bottom:12px; transition:all 0.2s; position:relative;';
                ...
                row.innerHTML = `
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                        ...
                        <div style="display:flex; align-items:center; gap:8px;">
                            ${isLast
                                ? '<span style="font-size:10px; color:#e41837; font-weight:700; text-transform:uppercase; letter-spacing:0.5px; margin-right:8px;">Mandatory Base</span>'
                                : `<div class="hierarchy-row-btns">
                                    <button class="hierarchy-row-btn remove" data-index="${i}" title="Remove Level">
                                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><line x1="5" y1="12" x2="19" y2="12"/></svg>
                                    </button>
                    ...
                    <select class="cet-input cet-select update-geo-select" data-level="${i + 1}" 
                        style="width:100%; font-size:14px; font-weight:600; border-color:#f1f3f5; border-radius:8px;
                               ${isLast ? 'background:#f8f9fa; border-color:#e5e7eb; color:#9ca3af;' : ''}"
                        ${isLast ? 'disabled' : ''}>
                ...
                if (select && !isLast) {
                    select.onchange = () => {
                        updCurrentLevels[i] = select.value;
                        const allValues = Array.from(container.querySelectorAll('.update-geo-select')).map(s => s.value);
                        renderUpdateSummary(allValues);
                        renderUpdateAttributes(allValues, ent.attributes);
                    };
                }
            });

            // Bind per-row add buttons
            container.querySelectorAll('.hierarchy-row-btn.add').forEach(btn => {
                btn.onclick = () => {
                    const idx = parseInt(btn.dataset.index);
                    if (updCurrentLevels.length < 6) {
                        updCurrentLevels.splice(idx + 1, 0, '');
                        renderUpdateUI([...updCurrentLevels]);
                    } else {
                        showCustomModal({ title: 'Limit Reached', message: 'Maximum 6 hierarchy levels allowed.', type: 'info' });
                    }
                };
            ...
            // Bind per-row remove buttons
            container.querySelectorAll('.hierarchy-row-btn.remove').forEach(btn => {
                btn.onclick = () => {
                    const idx = parseInt(btn.dataset.index);
                    if (updCurrentLevels.length > 2) {
                        updCurrentLevels.splice(idx, 1);
                        renderUpdateUI([...updCurrentLevels]);
                    } else {
                        showCustomModal({ title: 'Minimum Reached', message: 'At least 2 hierarchy levels are required.', type: 'info' });
                    }
